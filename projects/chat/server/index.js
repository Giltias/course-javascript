...
const server = http.createServer(async (req, res) => {
  try {
    if (/\/photos\/.+\.jpeg/.test(req.url)) {
      const [, imageName] = req.url.match(/\/photos\/(.+\.jpeg)/) || [];
      const fallBackPath = path.resolve(__dirname, '../no-photo.jpeg');
      ...
      const filePath = path.resolve(__dirname, '../photos', `${name}.jpeg`);
